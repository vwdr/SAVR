#!/usr/bin/env python3
"""Invoke the frozen V5-D runner through the isolated V08 adapters."""
...
def main() -> int:
    ...
    if backend == "raw-cudagraph" and allocator != EXPECTED_ALLOCATOR:
        raise SystemExit("V08 raw process lacks the exact inherited V07 allocator setting")
    if backend == "torch-compile" and allocator is not None:
        raise SystemExit("V08 compiler process must not receive an allocator override")

    from savr.acr.v5_d_v08_adapter import install_v08_adapters, set_active_lifecycle
    ...
    from savr.acr.v5_d_v08_runtime import V08_RUN_ID, load_v08

    runner.V03_RUN_ID = V08_RUN_ID
    if backend == "raw-cudagraph":
        import torch

        config = load_v08(ROOT)
        run_root = ROOT / "results" / V08_RUN_ID
        launch = json.loads((run_root / "launch" / "record.json").read_text(encoding="utf-8"))
        attempt = json.loads(
            (run_root / "backend-attempt-torch-compile" / "record.json").read_text(
                encoding="utf-8"
            )
        )
        permit = json.loads(
        ...
        if any(
            ...
            raise SystemExit("V08 transition provenance semantic hash mismatch")
        if (
            ...
            raise SystemExit("V08 transition lacks an authorized zero-output compiler permit")
        ...
        def guarded_attempt() -> int:
            try:
                ...
                lifecycle.close()
                record = {
                    "schema_version": "acr.v5d-inference-lifecycle.v8",
                    "run_id": V08_RUN_ID,
