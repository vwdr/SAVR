...
if [[ "$(pwd -P)" != "${project_root}" ]]; then
  echo "Refusing V5-D launch outside ${project_root}" >&2
  exit 1
fi
if [[ ! -x "${python_bin}" || ! -f "${manifest}" ]]; then
  echo "V5-D pinned interpreter or launch manifest is missing" >&2
  exit 1
fi
...
if [[ ${status} -eq 20 ]]; then
  "${python_bin}" scripts/run_acr_v5_d.py --backend raw-cudagraph
  status=$?
fi
if [[ ${status} -ne 0 ]]; then
  exit "${status}"
fi
"${python_bin}" scripts/finalize_acr_v5_d.py
