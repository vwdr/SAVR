#!/usr/bin/env python3
"""Invoke the frozen V5-D runner through V04 and V05 isolation adapters."""
...
def main() -> int:
    ...
    from savr.acr.v5_d_v05_runtime import V05_RUN_ID, load_v05
    from savr.acr.v5_d_v05_transition import V05TransitionSampler

    runner.V03_RUN_ID = V05_RUN_ID
    if requested_backend() == "raw-cudagraph":
        config = load_v05(ROOT)
        run_root = ROOT / "results" / V05_RUN_ID
        ...
        if any(record.get("semantic_sha256") != semantic_sha256(record) for record in (launch, attempt, permit)):
            raise SystemExit("V05 transition provenance semantic hash mismatch")
        if (
            ...
            raise SystemExit("V05 transition lacks an authorized zero-output compiler permit")
