#!/usr/bin/env python3
"""Closed-stdin, CUDA-hidden import and graph-pool API preflight for V5-D v04."""
...
PREFLIGHT_ID = "acr-v5d-v04-pool-api-import-preflight-v01"
...
def main() -> int:
    ...
    if root != EXPECTED_ROOT:
        raise SystemExit(f"Refusing V5-D v04 import preflight outside {EXPECTED_ROOT}: {root}")
    ...
    if git_output(upstream, "rev-parse", "HEAD") != OPENVLA_REVISION or git_output(
        ...
        raise SystemExit("V5-D v04 import preflight source revision changed")
    if git_output(upstream, "status", "--porcelain") or git_output(
        libero, "status", "--porcelain"
    ):
        raise SystemExit("V5-D v04 import preflight source tree is dirty")

    preflight_root = root / "results" / PREFLIGHT_ID
    if preflight_root.exists():
        raise SystemExit(f"Immutable V5-D v04 import preflight already exists: {preflight_root}")
...
def forbidden_input(*args, **kwargs):
    raise RuntimeError("V5-D v04 interactive input was attempted")
...
'''
    ...
    if completed.returncode != 0:
        raise RuntimeError("V5-D v04 closed-stdin import failed: " + completed.stderr[-2000:])
    ...
    if imported != expected:
        raise RuntimeError("V5-D v04 CUDA-hidden pool API attestation changed")
    record = {
        "schema_version": "acr.v5d-pool-api-import-preflight.v4",
        "status": "pass",
    ...
    write_json_once(root / "reports/runtime/acr_v5_d_v04_import_preflight.json", record)
