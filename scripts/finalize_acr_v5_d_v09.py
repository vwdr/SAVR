#!/usr/bin/env python3
"""Analyze V09 twice and publish independent verification."""
...
def main() -> int:
    ...
    if root != EXPECTED_ROOT:
        raise SystemExit(f"Refusing to finalize V5-D V09 outside {EXPECTED_ROOT}: {root}")
    ...
    if canonical_bytes(first) != canonical_bytes(second):
        raise RuntimeError("V5-D V09 repeated analysis is not byte-identical")
    errors = verify(config, run, first)
    if errors:
        raise RuntimeError(f"V5-D V09 independent verification failed: {errors}")
    store = ImmutableRecordStore(root / "results")
    store.write_once("acr-v5d-analysis-v09", first)
    verification = {
    ...
    store.write_once("acr-v5d-verification-v09", verification)
