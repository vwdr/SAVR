...
run_id="acr-v5d-real-tensor-feasibility-v10"
...
if [[ "$(pwd -P)" != "${project_root}" ]]; then
  echo "Refusing V5-D V10 launch outside ${project_root}" >&2
  exit 1
fi
if [[ ! -x "${python_bin}" || ! -f "${manifest}" ]]; then
  echo "V5-D V10 pinned interpreter or authorized launch manifest is missing" >&2
  exit 1
fi
if [[ -n "${PYTORCH_CUDA_ALLOC_CONF:-}" ]]; then
  echo "Refusing inherited allocator configuration" >&2
  exit 1
fi
...
"${python_bin}" scripts/prepare_acr_v5_d_v10_libero_config.py

set +e
"${python_bin}" scripts/run_acr_v5_d_v10.py --backend torch-compile
status=$?
set -e
if [[ ${status} -eq 20 ]]; then
  "${python_bin}" scripts/run_acr_v5_d_v10.py --backend raw-cudagraph
  status=$?
fi
if [[ ${status} -ne 0 ]]; then
  exit "${status}"
fi
"${python_bin}" scripts/finalize_acr_v5_d_v10.py
