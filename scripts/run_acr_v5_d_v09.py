#!/usr/bin/env python3
"""Invoke the frozen V5-D runner through the isolated V09 adapters."""
...
def main() -> int:
    ...
    if os.environ.get("PYTORCH_CUDA_ALLOC_CONF") is not None:
        raise SystemExit(f"V09 {backend} process must use the default allocator environment")
    ...
    from savr.acr.v5_d_v09_runtime import V09_RUN_ID, load_v09

    runner.V03_RUN_ID = V09_RUN_ID
    if backend == "raw-cudagraph":
        import torch

        config = load_v09(ROOT)
        run_root = ROOT / "results" / V09_RUN_ID
        ...
        if any(
            ...
            raise SystemExit("V09 transition provenance semantic hash mismatch")
        if (
            ...
            raise SystemExit("V09 transition lacks an authorized zero-output compiler permit")
        ...
        def guarded_attempt() -> int:
            try:
                ...
                lifecycle.close()
                record = {
                    "schema_version": "acr.v5d-inference-lifecycle.v9",
                    "run_id": V09_RUN_ID,
