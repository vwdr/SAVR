#!/usr/bin/env python3
"""Closed-stdin, CPU-only import preflight for the V5-D v02 LIBERO recovery."""
...
PREFLIGHT_ID = "acr-v5d-v02-libero-import-preflight-v01"
...
def main() -> int:
    ...
    if root != EXPECTED_ROOT:
        raise SystemExit(f"Refusing V5-D v02 import preflight outside {EXPECTED_ROOT}: {root}")
    ...
    if (
        ...
        raise SystemExit("V5-D v02 import preflight source revision changed")
    if git_output(upstream, "status", "--porcelain") or git_output(libero, "status", "--porcelain"):
        raise SystemExit("V5-D v02 import preflight source tree is dirty")

    preflight_root = root / "results" / PREFLIGHT_ID
    if preflight_root.exists():
        raise SystemExit(f"Immutable V5-D v02 import preflight already exists: {preflight_root}")
...
def forbidden_input(*args, **kwargs):
    raise RuntimeError("V5-D v02 interactive input was attempted")
...
"""
    ...
    if completed.returncode != 0:
        raise RuntimeError("V5-D v02 closed-stdin import failed: " + completed.stderr[-2000:])
    lines = [line for line in completed.stdout.splitlines() if line.strip()]
    if not lines:
        raise RuntimeError("V5-D v02 import preflight emitted no attestation")
    ...
    if imported != {
        ...
        raise RuntimeError("V5-D v02 import preflight attestation changed")
    record = {
        "schema_version": "acr.v5d-libero-import-preflight.v2",
        "status": "pass",
    ...
    write_json_once(root / "reports/runtime/acr_v5_d_v02_import_preflight.json", record)
