#!/usr/bin/env python3
"""Closed-stdin, CUDA-hidden import/API preflight for V5-D v06."""
...
PREFLIGHT_ID = "acr-v5d-v06-precapture-warmup-import-preflight-v01"
...
def main() -> int:
    ...
    if root != EXPECTED_ROOT:
        raise SystemExit(f"Refusing V5-D v06 import preflight outside {EXPECTED_ROOT}: {root}")
    ...
    if (
        ...
        raise SystemExit("V5-D v06 import preflight source revision changed")
    if git_output(upstream, "status", "--porcelain") or git_output(libero, "status", "--porcelain"):
        raise SystemExit("V5-D v06 import preflight source tree is dirty")

    preflight_root = root / "results" / PREFLIGHT_ID
    if preflight_root.exists():
        raise SystemExit(f"Immutable V5-D v06 import preflight already exists: {preflight_root}")
...
def forbidden_input(*args, **kwargs):
    raise RuntimeError("V5-D v06 interactive input was attempted")
...
print(json.dumps({
    ...
    "v06_backend_imported": V06PreCaptureWarmupRawCudaGraphCorePair.__name__ == "V06PreCaptureWarmupRawCudaGraphCorePair",
...
"""
    ...
    if completed.returncode != 0:
        raise RuntimeError("V5-D v06 closed-stdin import failed: " + completed.stderr[-2000:])
    ...
    expected = {
        ...
        "v06_backend_imported": True,
        "visible_device_count": 0,
    }
    if imported != expected:
        raise RuntimeError("V5-D v06 CUDA-hidden import/API attestation changed")
    record = {
        "schema_version": "acr.v5d-precapture-warmup-import-preflight.v6",
        "status": "pass",
    ...
    write_json_once(root / "reports/runtime/acr_v5_d_v06_import_preflight.json", record)
