#!/usr/bin/env python3
"""Analyze V04 twice and publish independent verification."""
...
def main() -> int:
    ...
    if root != EXPECTED_ROOT:
        raise SystemExit(f"Refusing to finalize V5-D v04 outside {EXPECTED_ROOT}: {root}")
    sys.path.insert(0, str(root / "src"))
    sys.path.insert(0, str(root / "scripts"))
    from analyze_acr_v5_d import analyze, canonical_bytes
    ...
    if canonical_bytes(first) != canonical_bytes(second):
        raise RuntimeError("V5-D v04 repeated analysis is not byte-identical")
    errors = verify(config, run, first)
    if errors:
        raise RuntimeError(f"V5-D v04 independent verification failed: {errors}")
    store = ImmutableRecordStore(root / "results")
    store.write_once("acr-v5d-analysis-v04", first)
    verification = {
    ...
    from analyze_acr_v5_d import semantic_sha256

    verification["semantic_sha256"] = semantic_sha256(verification)
    store.write_once("acr-v5d-verification-v04", verification)
