#!/usr/bin/env python3
"""Analyze V10 twice and publish independent verification."""
...
def main() -> int:
    ...
    if root != EXPECTED_ROOT:
        raise SystemExit(f"Refusing to finalize V5-D V10 outside {EXPECTED_ROOT}: {root}")
    ...
    run = json.loads(
        (root / "results" / config["run_id"] / "final" / "record.json").read_text(encoding="utf-8")
    )
    architecture_errors = verify_v10_architecture(run)
    if architecture_errors:
        raise RuntimeError(f"V5-D V10 architecture verification failed: {architecture_errors}")
    first = analyze(config, run)
    second = analyze(config, run)
    if canonical_bytes(first) != canonical_bytes(second):
        raise RuntimeError("V5-D V10 repeated analysis is not byte-identical")
    errors = verify(config, run, first)
    if errors:
        raise RuntimeError(f"V5-D V10 independent verification failed: {errors}")
    store = ImmutableRecordStore(root / "results")
    store.write_once("acr-v5d-analysis-v10", first)
    verification = {
        "schema_version": "acr.v5d-verification.v10",
    ...
    store.write_once("acr-v5d-verification-v10", verification)
