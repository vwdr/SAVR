...
\draw[dashed,gray] (0,98) -- (90,98);
\node[anchor=south east,text=gray] at (89,98) {SAVR1 $-2\pp$ gate};
\foreach \x/\y in {34.69/52,43.29/21,47.28/23,56.62/12,63.26/4,65.21/0,64.14/3,75/1,83.68/0}{
  \fill[BrickRed] (\x,\y) circle (2.1pt);
}
\node[anchor=south west,text=BrickRed] at (35.5,53) {SAVR1};
\node[draw=MidnightBlue,fill=MidnightBlue,rectangle,minimum size=4.5pt,inner sep=0pt] at (0,100) {};
\node[draw=ForestGreen,fill=ForestGreen,regular polygon,regular polygon sides=3,minimum size=5.5pt,inner sep=0pt] at (0,100) {};
...
\draw[->,thin,gray] (15,82) -- (4,95);
\node[anchor=west,text=gray] at (15,82) {zoomed at right};
\end{scope}

% Zoom of the high-success cluster (same exact coordinates, expanded axes).
...
\node[font=\small\bfseries,anchor=south west] at (0,14.3) {High-success region};
\draw[dashed,gray] (0,10) -- (12.5,10);
\node[draw=MidnightBlue,fill=MidnightBlue,rectangle,minimum size=5pt,inner sep=0pt] at (0,12) {};
\node[draw=ForestGreen,fill=ForestGreen,regular polygon,regular polygon sides=3,minimum size=6pt,inner sep=0pt] at (0,12) {};
...
\node[anchor=west] at (0.7,13.1) {FR + SAVR2-$b05$};
\node[anchor=west,text=RoyalPurple] at (1.5,10.6) {SAVR3};
\node[anchor=north,text=ForestGreen] at (6.72,8.05) {SAVR2-$b10$};
\node[anchor=east,text=ForestGreen] at (10.2,2) {SAVR2-$b15$};
\end{scope}
\end{tikzpicture}
\caption{Observed success-efficiency frontier with the crowded high-success region expanded at right. The coincident FR and zero-reuse SAVR2-$b05$ results share one coordinate. SAVR1 used 100 episodes per point, SAVR2 used 30 per point, and SAVR3 used 70; the plot is descriptive because the staged populations differ.}
\label{fig:frontier}
\end{figure}
